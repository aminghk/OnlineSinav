                        ...
                        <form className="boxes" onSubmit={e => this.register(e)}>
                            ...
                            <div className="box-tab information">
                                <InputForm
                                    ...
                                    label="email"
                                    placeholder="hasanbulut@ege.edu.tr" />
                                <InputForm
                                    tabIndex={5}
                                    value={phone}
                                    onChange={(val) => this.changeInfo('phone', val)}
                                    label="telefon"
                                    placeholder="5539099962" />
                            </div>
                        ...
                        <div className="wrap-next-btn">
                            {
                                (function () {
                                    if (currentTab === 1)
                                        return <ButtonPrimary handleClick={() => {this.register(); this.changeTab(null, 'next')}}>Kayıt</ButtonPrimary>
                                    else if (currentTab === 2)
                                        return <Link to='/'><ButtonPrimary>Ana sayfa</ButtonPrimary></Link>
                                    else
                                        return <ButtonPrimary handleClick={() => this.changeTab(null, 'next')} >Sonrakı adım</ButtonPrimary>
                                })()
                            }
                        </div>
                    </div>
